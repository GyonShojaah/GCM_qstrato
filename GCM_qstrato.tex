...
Over \memoYF{a decade}, various three-dimensional (3D) general circulation models (GCMs) revisited the atmospheric structure of terrestrial planets and have demonstrated the importance of accounting for the 3D circulation and climate heterogeneity \citep[e.g.][]{Ishiwatari2002,Abe2011,Leconte2013a,Leconte2013b,Wolf2014,Wolf2015}. 
%
\memoYF{In particular, planets in the habitable zones around late-type stars may be synchronously rotating due to the tidal effect \citep{Dole1964, Kasting1993}, although this need not always occur \citep{Goldreich1966,Leconte2015}.
In the case of the synchronous rotation, the climate can not be easily captured by 1D models \citep{Leconte2013b}. }  
%
...
%
\memoYF{While the model details are presented in \citet{Way2017}, here we supplement with a brief description of the \modelE{} cloud scheme in Appendix \ref{ap:clouds}, because it is one of the major uncertainties in any GCMs, and because it affects our results as we will see below. }
%
...
\memoYF{The version of the model used here is the same as that used by \citet{Way2016}, except for the radiation scheme. It } 
...
\memoYF{The optical properties of liquid clouds are derived using Mie theory, assuming XXX size distribution (?) with the effective radius, diagnosed from the cloud water content assuming a specified number concentration for liquid clouds over ocean. 
The optical properties ice crystals are based on the representation of ice aggregates with the effective dimension \citep{Baran2001} which is a function of temperature, as described in \citep{Edwards2007}. 
...
%
The vertical cloud overlap is treated using the mixed maximum-random overlap assumption, that is, clouds in adjacent layers overlap maximally, while clouds separated by one or more layers overlap randomly.}
...
\memoYF{We also run equivalent simulations for other types of stars and found the overall responses to the ``transparent high clouds'' assumption are very similar.}
...
\memoYF{Although the enhanced abundance of \wv{} was also observed in 1D photochemical atmospheric models \citep{Rauer2011,Rugheimer2013,Rugheimer2015}, the mechanism that is responsible for \wv{} is different because we did not included photochemical production of \wv{}.}{}
...
Detrained condensate is treated by the stratiform cloud microphysics (an updated version of \citet{DelGenio1996}). 
