...
%
Here we study the 3-dimensional distributions of \water for synchronously rotating aquaplanets using the general circulation model {\it ROCKE-3D} and examine the effect of total incident flux and the spectral type of the star.  
%
...
%%%
\begin{equation}
\frac{d\Theta }{dt} + u \frac{d\Theta }{dx} + v \frac{d\Theta }{dy} + w \frac{d\Theta }{dz} = \frac{ Q_r + Q_l }{\rho c_p}. \label{eq:conserv_energy_2}
\end{equation}
%%%
where $Q_r$ and $Q_l$ are the radiative and latent heating rates, $c_p$ is the specific heat of the atmosphere, $\rho $ is atmospheric density, $\{ u, v, w \}$ are the conventional velocity components toward the east ($x-$axis), north ($y-$axis), and upward ($z-$axis), respectively. 
