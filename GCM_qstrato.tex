...
Because the upper atmosphere is stable against convection, i.e., $\partial \Theta / \partial p < 0 $, larger (positive) $Q_r$ leads to a larger absolute value for $\omega $ (with negative sign), which means that stronger upward motion develops. 
...
In order to isolate the effect of rotation, we performed experiments where we varied the spin period while keeping the incident flux fixed at $S_X=1$ with the GJ~876 spectrum, but allowing the planet to remain in synchronous rotation. 
...
\yf{The spin effect is non-negligible, though;} we find that the \xwater{} becomes smaller as the spin period gets very short\yf{, i.e., increasing planet spin partly offsets the effect of increasing stellar flux.}  %, most prominent at spin period smaller than 30 days. 
...
As the spin period becomes shorter, however, the atmosphere develops upper level zonal winds, as demonstrated in previous papers \citep{Merlis2010, Edson2011, Kopparapu2016}. 
%The transition is characterized by the Rossby deformation radius larger than planetary radius, which is .  
%
The super-rotating zonal wind seen in the equatorial region in the shorter period runs (not shown) efficiently \yf{transports water vapor from the dayside to the nightside and limits cloudiness in the substellar region}.
...
Consistently, in the runs with the shorter spin period, \yf{the convection at the substellar point is deeper, and the temperature at the top of the convection where it moistens the atmosphere is colder, and thus the convection is less able to moisten the atmosphere there.}. 
