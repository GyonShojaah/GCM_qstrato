...
%\def\memoYF#1{{\bf #1}}
\def\memoYF#1{\color{red}#1\color{black}}
\newcommand{\dsa}[1]{{\bf#1}}
...
Detectability of H$_2$O absorption features in transmission spectra depends on the \memoYF{abundance}{mixing ratio} of water vapor in the upper part of the atmosphere. 
...
Here we study the three-dimensional distribution of atmospheric H$_2$O for synchronously rotating \memoYF{Earth-size} aquaplanets using the general circulation model ROCKE-3D, and examine the effects of total incident flux and stellar spectral type.  
We observe a more \memoYF{gentle dependence on}{gradual increase } of water vapor mixing ratio in response to increased incident flux than one-dimensional models suggest, in qualitative agreement with the climate-stabilizing effect of clouds around the substellar point previously observed in GCMs applied to synchronously rotating planets. 
...
They found a transition from an Earth-like dry upper atmosphere to a water-dominated atmosphere, driven by the increase of the surface temperature, with only a 20\% increase in instellation relative to the solar constant. 
...
Over \memoYF{\dsa{the last} decade}, various three-dimensional (3D) general circulation models (GCMs) revisited the atmospheric structure of terrestrial planets and have demonstrated the importance of accounting for the 3D circulation and climate heterogeneity \citep[e.g.][]{Ishiwatari2002,Abe2011,Leconte2013a,Leconte2013b,Wolf2014,Wolf2015}. 
%
\memoYF{In particular, planets in the habitable zones around late-type stars may be synchronously rotating due to \dsa{tidal effects} \citep{Dole1964, Kasting1993}, although this need not always occur \citep{Goldreich1966,Leconte2015}.
In the case of the synchronous rotation, the climate cannot be easily captured by 1D models \citep{Leconte2013b}. }  
%
\citet{Yang2013} pointed out that for synchronously rotating planets, the convection around the substellar point produces optically thick clouds which increase the planetary albedo and contribute to keep the surface temperature moderate for a much wider range of instellation \citep[see also][]{Yang2014,Way2015,Kopparapu2016}. 
...
%
\memoYF{While the model details are presented in \citet{Way2017}, \dsa{a brief description of the \modelE{} cloud scheme can be found} in Appendix \ref{ap:clouds}, \dsa{as} it is one of the major uncertainties in any \dsa{GCM}, and because it affects our results as we will see below. }
%
%\memoYF{Because one of the major uncertainties in any GCMs is the cloud scheme, we give a brief description of the \modelE{} cloud scheme in Appendix \ref{ap:clouds} for completeness, although it is also discussed in \citet{Way2017}. }


\memoYF{The version of the model used here is the same as that used by \citet{Way2016}, except for the radiation scheme. \dsa{The original ModelE2 radiation scheme}} 
...
\memoYF{The optical properties of liquid clouds \dsa{were derived using Mie theory, with the effective radius} diagnosed from the cloud water content assuming a specified number concentration for liquid clouds over ocean. 
\dsa{The optical properties of ice crystals is described in \citet{Edwards2007}, and are based on the representation of ice aggregates introduced by \citet{Baran2001}, with the effective dimension calculated using a parametrization based on temperature.}
...
\dsa{Vertical cloud overlap is treated using the maximum-random overlap assumption, where clouds in adjacent layers overlap maximally, while clouds separated by one or more layers overlap randomly.} 
}
...
%
The third and fourth points above imply that the effects of clouds we find are the largest among the possible realizations \memoYF{given the particle size distribution, because of the non-linear dependence of the on transmittance on the optical thickness}. 
...
Above 1 mbar, which is located \memoYF{between} 35 and 50 km altitude depending on the atmospheric temperature profiles, the \wv{} mixing ratio becomes horizontally uniform within a few tens of percent. 
...
\begin{figure*}[!tb]
    ...
    \end{center}
\caption{Water vapor mixing ratio at \preslevel{} (\xwater{}) as a function of total incident flux (left), the surface temperature averaged within $0.2R_\earth$ of the substellar point (center), and the near-infrared portion of the incident flux integrated \memoYF{between} 0.9 and 3.0 \textmu m (right). The colors represent the stellar types: Sun (navy), HD~22048 (green), Kepler-186 (orange), and GJ~876 (red). Dashed lines in the left panel are adapted from \memoYF{the 1D model of \citet{Kopparapu2013} for varying stellar effective temperatures: 5800~K (navy), 4800~K (green), and 3800~K (orange). The dashed black line in the middle panel is also adopted from \citet{Kopparapu2013}. }}           
\label{fig:xH2O_S0X}
\end{figure*}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
This large-scale circulation can be explained as follows: When the incident flux increases, the heating rate due to the absorption of short-wave radiation by \wv{}, mainly in the near-infrared (NIR) regime, also increases. 
...
\memoYF{We also run equivalent simulations for other types of stars and found \dsa{that} the overall responses to the ``transparent high clouds'' assumption are very similar.}
...
\begin{figure*}[htb]
    \begin{center}
    \includegraphics[width=1\hsize]{vectormap_altitude-longitude_GJ876_Sun_z_1000.pdf}
    \end{center}
\caption{}
\label{fig:vectormap}
\end{figure*}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Effect of Other Planetary Parameters}
\label{ss:sensitivity_planetparam}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%	

\memoYF{%
In principle, water vapor distributions also depend on geophysical parameters including the atmospheric pressure, atmospheric composition, surface gravity, and the planetary surface water coverage, as well as photochemistry, which depends on the detailed atmospheric profiles. 
For example, 
The dependence of the water vapor mixing ratio on these planetary properties will be the focus of future study. %
}
...
\memoYF{Although the enhanced abundance of \wv{} \dsa{has} also observed in 1D photochemical atmospheric models \citep{Rauer2011,Rugheimer2013,Rugheimer2015}, the mechanism that is responsible for \wv{} is different because we did not included photochemical production of \wv{}.}{} \dsa{@@@ I do not think it is entirely clear here what you mean by ``responsible for \wv{}''}
...
In principle, however, \wv{} distributions also depend on geophysical parameters including the surface gravity, atmospheric composition, atmospheric pressure \memoYF{\citep[e.g.,][]{Turbet2016}}, and the planetary water abundance, as well as photochemistry, which depends on the detailed atmospheric profiles. 
...
\memoYF{The \modelE{}  cumulus parameterization uses a mass flux closure determined by the mass required to establish neutral buoyancy at cloud base over a specified adjustment time.  The total mass flux is divided into two components (``plumes'') that entrain at different rates \citep{DelGenio2007}. 
...
Detrained condensate is treated by the stratiform cloud microphysics \dsa{\citep[an updated version of][]{DelGenio1996}}. 
...
\includegraphics[width=\hsize]{ANN0099-0134aijkAqOH0TLS_GJ876S06P34L40Q_SW29nc_z_5.pdf}
    \end{minipage}
    \begin{minipage}{0.24\hsize}
\includegraphics[width=\hsize]{ANN0132-0175aijkAqOH0TLS_GJ876S08P27L40Q_SW29nc_z_5.pdf}
    \end{minipage}
    \begin{minipage}{0.24\hsize}
\includegraphics[width=\hsize]{ANN0099-0150aijkAqOH0TLS_GJ876S10P23L40Q_SW29nc_z_5.pdf}
    \end{minipage}
    \begin{minipage}{0.24\hsize}
\includegraphics[width=\hsize]{ANN0077-0137aijkAqOH0TLS_GJ876S12P20L40Q_SW29nc_z_5.pdf}
    \end{minipage}
\caption{test}
\end{figure*}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{figure*}[tb]
    \begin{minipage}{0.24\hsize}
\includegraphics[width=\hsize]{ANN0007-0010aijkAqOH0TLS_SunS14P284L40Q_SW29nc_z_5.pdf}
    \end{minipage}
    \begin{minipage}{0.24\hsize}
\includegraphics[width=\hsize]{ANN0008-0012aijkAqOH0TLS_SunS18P236L40Q_SW29nc_z_5.pdf}
    \end{minipage}
    \begin{minipage}{0.24\hsize}
\includegraphics[width=\hsize]{ANN0010-0013aijkAqOH0TLS_SunS22P202L40Q_SW29nc_z_5.pdf}
    \end{minipage}
    \begin{minipage}{0.24\hsize}
\includegraphics[width=\hsize]{ANN0012-0015aijkAqOH0TLS_SunS26P178L40Q_SW29nc_z_5.pdf}
    \end{minipage}
\caption{test}
\end{figure*}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
