...
\addYF{Besides, while \water{} } is a dominant opacity source of Earth's atmosphere, \addYF{its signatures in the transmission spectra of temperate planets are not strong} due to the ``cold trap'', i.e., \wv{} evaporated from the surface is transported upward, but condenses as air rises and cools, and most of it precipitates, leaving the stratosphere\addYF{---where transmission spectroscopy typically probes---}fairly dry. 
Several studies modeling transmission spectra of the Earth found \addYF{modest} signatures of \wv{} \citep[e.g.][]{Ehrenreich2006, Kaltenegger2009, Betremieux2013, Misra2014}. 
...
%
%\addYF{In particular, planets in the habitable zones around late-type stars may be synchronously rotating due to tidal locking \citep{Dole1964, Kasting1993}, although this need not always occur \citep{Goldreich1966,Leconte2015}.
%In the case of synchronous rotation, the climate cannot be easily captured by 1D models \citep{Leconte2013b}. }  
%
...
Motivated by these works on the stabilized climate of \addYF{synchronously rotating planets}, we are specifically interested in the \wv{} transport into the upper atmosphere, not only because it affects the water loss rate (and thus the conventional criterion for habitable conditions) but also because it limits our ability to detect \water{} signatures of planets with surface liquid water, through transmission spectroscopy. 
%
Indeed, planets around later type stars, \addYF{which are more likely to be synchronously rotating,} are best suited for future transit observations for several reasons, including the higher transit probability, larger signals in transmission spectra, and the abundance of late-type stars in the nearby universe. 
...
\addYF{The organization of this paper is as follows. %
Section \ref{s:model} describes the set-up of our GCM experiments and of simulating transmission spectra. %
Section \ref{s:results} discusses our results of \wv{} profiles as a function incident spectra, and shows the modeled transmission spectra. %
The sensitivities of our results to the ocean model and the CO$_2$ abundance as well as the effect of the spin period are studied in Section \ref{s:sensitivity}. Finally, Section \ref{s:summary} summarizes our findings. % 
}
...
%\addYF{Of these, the most important for our results are the radiation parameterization and the convection/cloud parameterizations.  
%We therefore discuss each of these in some detail below, followed by the description of other settings specific to our experiments in this paper. }
%
%\addYF{Because one of the major uncertainties in any GCMs is the cloud scheme, we give a brief description of the \modelE{} cloud scheme in Appendix \ref{ap:clouds} for completeness, although it is also discussed in \citet{Way2017}. }
...
% The original ModelE2 radiation scheme } 
% the radiation scheme
%has been replaced by the Suite of Community RAdiative Transfer codes based on Edwards and Slingo\footnote{\url{https://code.metoffice.gov.uk/trac/socrates}}~\citep[SOCRATES,][]{EdwardsSlingo1996,Edwards1996} as described in \citet{Way2017}. 
...
% is described in Edwards et al. (2007), and is based on the representation of ice aggregates introduced by Baran et al. (2001), with the effective dimension De of ice aggregate particles calculated using a parametrization of De as a function of temperature. 
...
% since loosening these assumptions makes the effective cloud optical thickness smaller. 
...
\addYF{Instead, we find that the near-IR component of the incident flux appears to be a good predictor of stratospheric humidity across the full range of stellar spectral types (Figure~\ref{fig:xH2O_S0X}, the right panel). %
...
% Their cooperation eventually finds a steady state where $\partial \theta/\partial t = 0$, leaving a stable circulation in the upper atmosphere around the substellar point. 
...
Here, the vertical components of the vectors are multiplied by 1000 \addYF{to make them more visible, because large-scale vertical motions are about three orders of magnitude weaker than horizontal motions.} 
%(In other words, the direction of the vectors are not real.) 
% However, the lengths of the arrows and the colors show $\sqrt{ v_x^2 + v_z^2 }$. 
%Note that we did not use the conventional stream lines in the pressure-latitude plane because we study synchronously rotating planets where dayside-nightside circulation dominates. 
The development of the large-scale circulation above the troposphere is clearly seen. 
Figures \ref{fig:heating_rate} and \ref{fig:vectormap} show that the stratospheric dynamical response to increased stellar heating is similar for stars of different spectral type, but for a warmer star the onset of strong upward motion occurs at a much larger incident flux because less of the instellation is in the near-IR.  This is the reason for the behavior seen in the right panel of Figure~\ref{fig:xH2O_S0X}.
...
\addYF{We also ran equivalent simulations for other types of stars and found that the overall effects of the ``transparent high clouds'' assumption are very similar.}
...
\begin{figure}[htb]
    ...
    \end{center}
\caption{\addYF{Velocity fields along the equator in the cases of low (left) and high (right)irradiance, for GJ876 (top) and the Sun (bottom). The vertical components of the vectors are multiplied by 1000 for display purposes, while the colors show the absolute values, $\sqrt{v_x^2 + v_z^2} \sim  v_x$. }}
\label{fig:vectormap}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
In this subsection, we show the \addYF{the dependence of the} transmission spectra \addYF{on the incident flux, taking} the modeled planets around one of our M-type stars, GJ~876 \addYF{as examples}. 
...
This level of effective altitude of water signatures is comparable with the strongest features in the modeled visible-to-NIR transmission spectrum of the Earth \addYF{shown in the black line}. % \citep[e.g.,][]{Kaltenegger2009}.
...
\addYF{Compared to the low-irradiation case (\wv{} mixing ratio $\sim 10^{-6}$) with the same assumptions for the cloud optical properties, the signal of a high-irradiation case would be larger by a factor of a few. }

\addYF{%We do not discuss in detail the detectability of individual systems considered in this paper, because is depends significantly on the distance from the Earth, the instrumental properties and the observation strategy. 
%Instead, 
...
\addYF{For a reference, the black line shows the modeled transmission spectrum of the Earth based on the US standard atmosphere. }}
...
\addYF{We find a quasi-exponential increase of \wv{} mixing ratios in the upper atmosphere as a function of total incident flux, resulting in a variation of several orders of magnitude as the total incident flux increases by a factor of two to three,  but much more gradual than predicted by 1-D models. }
...
\addYF{The increase of \wv{} mixing ratio in response to larger incident flux is not primarily caused by the increased surface temperature as previous 1D models suggest, due to the relatively stable surface temperature at the substellar point. }
Instead, it is induced by the effect of the increased NIR portion of the incident flux on the high-altitude circulation. 
The enhanced radiative heating due to the absorption by \wv{} and cloud particles drives a large-scale \addYF{circulation} of the air at altitudes above those to which convection penetrates, taking over the role of transporting \wv{} from the moist lower atmosphere to the upper atmosphere, and further increasing the heating rate in the upper atmosphere.
...
%\addYF{Although the enhanced abundance of \wv{} in the upper atmosphere has also been observed in 1D photochemical atmospheric models \citep{Rauer2011,Rugheimer2013,Rugheimer2015}, the mechanism that controls \wv{} is different because we did not include photochemistry. }
...
\addYF{We also find the presence of high clouds at the terminator, which}   potentially increases the baseline effective altitude and thus weakens the absorption features. 
\addYF{Our calculation may overestimate the effects of clouds on the transmission spectra due to the spatially and temporally averaged cloud properties of individual GCM grid cells.} Additional uncertainties come from other assumptions we made. 
\addYF{A more realistic inclusion of clouds in our simulated transmission spectra will be the scope of future work. }
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Cloud scheme in our GCM}
\label{ap:clouds}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\addYF{The \modelE{}  cumulus parameterization uses a mass flux closure determined by the mass required to establish neutral buoyancy at cloud base over a specified adjustment time.  The total mass flux is divided into two components (``plumes'') that entrain at different rates \citep{DelGenio2007}. 
Entrainment is proportional to parcel buoyancy and inversely proportional to the square of updraft speed, following \citet{Gregory2001}. 
Convection is triggered when a parcel lifted one model layer becomes buoyant according to a virtual moist static energy test. The plume rises until the updraft speed decreases to zero. Mass is detrained at all levels above the level of neutral buoyancy. 
A downdraft forms at any level when an equal mixture of cloud and environment is negatively buoyant. Updraft convective condensate is partitioned into fractions that precipitate, detrain, and are carried upward by assuming a Marshall-Palmer particle size distribution and empirical size-fall speed relationships and calculating the fraction of the size distribution whose fall speed exceeds, is comparable to, or is less than the diagnosed updraft speed \citep{DelGenio2005}. 
